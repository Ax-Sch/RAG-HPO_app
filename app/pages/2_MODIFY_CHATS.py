...
def validade_base_name(selected_base_name, valid_base_filenames=base_filenames):
    if selected_base_name in valid_base_filenames:
        return selected_base_name
    else:
        raise ValueError("Invalid base filename selected.")
...
if st.button("Update Text") and enable_edit:
    write_txt_file(validade_base_name(selected_base_name), updated_content, directory)
...
if st.button("Update Name") and new_name and enable_edit:
    if new_name not in base_filenames:  # Ensure the new name doesn't already exist
        rename_files(validade_base_name(selected_base_name), new_name, directory)
...
if st.button("Remove") and enable_edit:
    ...
    delete_files(validade_base_name(selected_base_name), directory)
